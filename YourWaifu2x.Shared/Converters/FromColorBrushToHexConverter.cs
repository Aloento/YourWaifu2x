...
namespace YourWaifu2x.Converters
{
    public class FromColorBrushToHexConverter : IValueConverter
    {
        ...
        public static string GetHexName(object value)
        {
            ...
            {
                string colorHex = "#" + brush.Color.R.ToString(HexadecimalStringFormat) + brush.Color.G.ToString(HexadecimalStringFormat) + brush.Color.B.ToString(HexadecimalStringFormat);
                if (brush.Opacity != 1)
                {
                    // Shows " 50%"
